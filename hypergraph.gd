class_name Hypergraph
extends RefCounted

# Incidence matrix: rows = nodes, columns = hyperedges
# Value 1 means node is in hyperedge, 0 means not
...
func apply_rewrite_rule_at_node(lhs: Hypergraph, rhs: Hypergraph, anchor_node: int, lhs_anchor: int = 0, rhs_anchor: int = 0) -> bool:
...
func _remove_pattern_at_node(pattern: Hypergraph, anchor_node: int, pattern_anchor: int) -> Array:
	var removed_edges = []
...
func _add_pattern_at_node(pattern: Hypergraph, anchor_node: int, pattern_anchor: int):
...
static func create_rule(lhs_nodes: int, lhs_edges: Array, rhs_nodes: int, rhs_edges: Array) -> Dictionary:
	var lhs = Hypergraph.new(lhs_nodes, 0)
	for edge in lhs_edges:
		lhs.add_hyperedge(edge)
	
	var rhs = Hypergraph.new(rhs_nodes, 0)
